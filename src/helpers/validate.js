    ...
    paramsNames.forEach((paramName) => {
        ...
        if (suitcase && metalDetector.range) {
            if (suitcase < metalDetector.range[0]) {
                return errorList.push(`"${paramName}" deve ser maior que ${metalDetector.range[0]}`);
            }
            if (suitcase > metalDetector.range[1]) {
                return errorList.push(`"${paramName}" deve ser menor que ${metalDetector.range[1]}`);
            }
        }

        if (suitcase && metalDetector.maxLength) {
            if (suitcase.length > metalDetector.maxLength) {
                return errorList.push(`"${paramName}" deve ser conter menos que ${metalDetector.maxLength} caracteres`);
            }
        }
